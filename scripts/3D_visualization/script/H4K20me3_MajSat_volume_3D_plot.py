...
# preset color for different chromosomes or gene families, transfer the Hex code to RGB code since Mayavi only take RGB color
...
# get the whole field of view IF voxel intensity and segmentation mask, and read in DNA loci coordinates
load_npz = np.load(str(input_path / IF_file))
ms_arr = load_npz["MajSat"]
h4_arr = load_npz["H4K20me3"]
ma_arr = load_npz["Mask"]
...
pad = 2 # set a pad to avoid half dot
...
sub_arr = np.copy(ms_arr[zoom]).astype(float)
...
sub_arr1 = np.copy(h4_arr[zoom]).astype(float)
sub_mask = np.copy(ma_arr[zoom])
...
# color for H4K20me3 bin chromosome in RBG (0, 255) version
...
# synchronize cameras in all figures
