...
@everywhere begin
    ...
    get_cell_contacts(x :: Tuple{DataFrame, Int64, Dict{String7, Int64}, Int64}) = get_cell_contacts(x...)
    
    function get_cell_contacts(points, radius, chrm_start_m1_dict, ind_max)
        ...
        contacts = spzeros(UInt16, ind_max, ind_max)

        for (pnt_ind, pnt_nbrs) in enumerate(pnts_nbrs)
            g_pnt_ind = chrm_start_m1_dict[points[pnt_ind, :chrom]] + points[pnt_ind, :g]
            for nbr_ind in pnt_nbrs
                if pnt_ind != nbr_ind
                    g_nbr_ind = chrm_start_m1_dict[points[nbr_ind, :chrom]] + points[nbr_ind, :g]
    ...
    function get_strand_pw(points, r,  chrm_start_m1_dict, ind_max)
        println("cell: ", points.finalcellID[1])
        freq_norm = zeros(UInt16, ind_max, ind_max)
...
all_contacts = zeros(UInt16, ind_max, ind_max)
for mat in mats
    all_contacts .+= mat
end
mats = []
