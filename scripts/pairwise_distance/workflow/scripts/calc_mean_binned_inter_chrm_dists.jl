...
@everywhere begin
    ...
    function get_chrm_chrm_dists(chr1, chr2, ncells)
        ...
        dists = 0 #zeros(Float64, max_chr1, max_chr2)
        nsums = 0 #zeros(Float64, max_chr1, max_chr2)
        for cell in 1:ncells
            println("cell: ", cell)
            cell_dists, cell_nsums = get_cell_chrom_dists(chr1[chr1.finalcellID .== cell,:], chr2[chr2.finalcellID .== cell,:])
            dists += cell_dists
            nsums += cell_nsums
        end
    ...
    function get_cell_chrom_dists(chr1, chr2)
        dists = 0 #zeros(Float64, 20, 20)
        nsums= 0 #zeros(Float64, 20, 20)
        temp = Float64[0,0,0]
        dist = 0.0
...
img = zeros(UInt16, nchroms, nchroms)
