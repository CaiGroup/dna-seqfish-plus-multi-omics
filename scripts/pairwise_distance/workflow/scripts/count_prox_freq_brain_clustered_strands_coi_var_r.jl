...
for (i, clusters) in enumerate(clusters_of_interest)
    ...
    strand_grps = groupby(cluster_pnts, [:finalcellID, :allele])
    ...
    for i in 1:nstrands
        all_contacts .+= get_cell_contacts(strand_dfs[i], search_radius, max_g)
        all_norm .+= get_strand_pw(strand_dfs[i], search_radius, max_g)
    end
