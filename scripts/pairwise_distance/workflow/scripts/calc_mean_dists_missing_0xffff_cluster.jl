...
for pair_mat in mats
    ...
    println("size chunk: ", size(img[ng_chroms.start_ind[i]:ng_chroms.end_ind[i], ng_chroms.start_ind[j]:ng_chroms.end_ind[j]]))
    ...
    if i != j
        println("size transpose chunk: ", size(img[ng_chroms.start_ind[j]:ng_chroms.end_ind[j], ng_chroms.start_ind[i]:ng_chroms.end_ind[i]] .= round.(UInt16, dists')))
