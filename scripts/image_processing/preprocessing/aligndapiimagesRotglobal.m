...
% ytakei@caltech.edu
    ...
    for folder = 1:length(folderArray)
            ...
            if folder == 1
                ...
                if size(newIm,3)>size(newImRef,3)
                    tformDapiRot = grabtformRot(newIm(:,:,size(newImRef,3)), newImRef, initialRadius, numIterations);
                else
            ...
            tformDapiUse.T(4,3) = 0; % need to comment out.
            for c = 1:numHybCh
    ...
    if size(dapiImsCheck{1},3)<size(dapiImsCheck{2},3)
        dapiImsCheck{2} = dapiImsCheck{2}(:,:,1:size(dapiImsCheck{1},3));
        dapiImsCheck{3} = dapiImsCheck{3}(:,:,1:size(dapiImsCheck{1},3));
    end
