function [pointsch, offsets] = alignpointswrapper_v3(chArray, pointsPath, offsetsPath, chaTform, numRounds, folderArray, physicalTforms, usechabboffsets, usephysicaloffsets)
    ...
    if length(offsets) == 2 || size(offsets{1},1) ~= size(offsets{1},3)% make third channel offset with second channel offset if offsets were calculated with two channels
    ...
    for c = chArray
        offsets{c}.row(:) = -offsets{c}.y(:) - ref_row ;
        offsets{c}.col(:) = offsets{c}.x(:) - ref_col;
