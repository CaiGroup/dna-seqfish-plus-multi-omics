function [chaTformInitial, chaTformFinal, physicaltform, refPointsAlignedInitial, ...
    refInitialSaveName] = debugbeads_cellnum_allCh_seg(experimentDir, posArray, numChannels, sizeZ, ...
...
% points using the roi to filter points in cells.
    ...
    segFolderName = 'HybCycle_96';
    segFolderNameSave = 'hyb-points';
    ...
    for position = chaTformArray
        roiPath = fullfile(experimentDir, 'segmentation', ['pos' num2str(position)], 'cellnum.mat');
        if exist(roiPath, 'file') == 2
            roimask = load(roiPath,'cellnum');
            roimask = roimask.cellnum;
            roimask(roimask > 0) = 1;
        else
            roimask = repmat(true(2048),[1 1 sizeZ]);
        end
        ...
        [chaTformInitialAll{position+1}, refPointsAlignedInitial{position+1}, ...
            ...
            getrefbeadsfilter_allCh(experimentDir, position, initFolderName, roimask, ...
        ...
        [chaTformFinalAll{position+1}, refPointsAlignedFinal{position+1}, ...
            ...
            getrefbeadsfilter_allCh(experimentDir, position, finalFolderName, roimask, ...
            superres, manualThreshold, experimentLabel, firstThreshold, numFirstPoints);
        [chaTformSegAll{position+1}, refPointsAlignedSeg{position+1}, ...
            intensitySeg{position+1}, firstThreshold, numFirstPoints] = ...
            getrefbeadsfilter_lastCh(experimentDir, position, segFolderName, roimask, ...
            superres, manualThreshold, experimentLabel, firstThreshold, numFirstPoints);
        for ch = 1:numCh
            if position == 0
    ...
    for position = chaTformArray
        ...
        endingString = [segFolderNameSave '-' experimentLabel];
        [refPointsSeg{position+1}, ~] = outputrefpoints(refPointsAlignedSeg{position+1}, chaTformSeg, intensitySeg{position+1}, position, endingString, saveDir, numCh);
    end
