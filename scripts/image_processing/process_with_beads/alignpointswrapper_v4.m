function [pointsch, offsets] = alignpointswrapper_v4(chArray, pointsPath, offsetsPath, chaTform, numRounds, folderArray, physicalTforms, usechabboffsets, usephysicaloffsets)
    ...
    for c = chArray
        offsets{c}.row(:) = -offsets{c}.y(:) + offsets{c}.y(1) ; % align to hyb1 of each channel as chromatic shift is already corrected in the latest version.
        offsets{c}.col(:) = offsets{c}.x(:) - offsets{c}.x(1);
        offsets{c}.z(:) = offsets{c}.z(:) - offsets{c}.z(1);
    end
