...
% Author: Nico Pierson
    ...
    if numvarargs == 1
        ...
        elseif ~strcmp(varargin{1}, 'roi') && ~strcmp(varargin{1}, 'cellnum') && ~strcmp(varargin{1}, 'whole') ...
        ...
        for r = 1:numRounds
            ...
            for c = 1:numChannels
                switch segment
                    ...
                    case 'cellnum'
                        % not a sophisticated way but fit with previous format
                        cellnum2 = cellnum==i; % logical
                        indices = find(round(points{r}(c).channels(:,2))>size(cellnum,1)|round(points{r}(c).channels(:,2))<1|round(points{r}(c).channels(:,1))>size(cellnum,2)|round(points{r}(c).channels(:,1))<1|round(points{r}(c).channels(:,3))>size(cellnum,3)|round(points{r}(c).channels(:,3))<1|isnan(round(points{r}(c).channels(:,2)))|isnan(round(points{r}(c).channels(:,1)))|isnan(round(points{r}(c).channels(:,3))));  
                        points{r}(c).channels(indices, :) = []; % remove outside image points
                        points{r}(c).intensity(indices, :) = []; % remove outside image points
                        points{r}(c).scaledIntensity(indices, :) = []; % remove outside image points
                        linInd = sub2ind(size(cellnum2),round(points{r}(c).channels(:,2)),round(points{r}(c).channels(:,1)),round(points{r}(c).channels(:,3)));
                        polygonIn = cellnum2(linInd);
                        pointsTemp{r}(c).channels = points{r}(c).channels(polygonIn,:);
                        pointsTemp{r}(c).intensity = points{r}(c).intensity(polygonIn,:);
                        pointsTemp{r}(c).scaledIntensity = points{r}(c).scaledIntensity(polygonIn,:);
                        points{r}(c).channels(polygonIn, :) = []; % remove used points
