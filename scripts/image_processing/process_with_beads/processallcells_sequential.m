function processallcells_sequential(projectDir, projectName, ...
...
% Author: Nico Pierson
    ...
    projectSaveName = [projectName '-Pos' num2str(position) '-' dateSaveString];
    ...
    if exist(segmentPath, 'file')
        switch segment
            ...
            case 'cellnum'
                % use labeled images from ilastik or cellpose
                [pointsxcell, numCells, numpointspercell] = segmentpoints2cells(segmentPath, points, segment);
            case 'whole'
                [pointsxcell, numCells, numpointspercell] = segmentpoints2cells(segmentPath, points, segment);
    ...
    savePointsFilePath = fullfile(savePath, ['pointsList-pos' num2str(position) '-' experimentLabel '-' dateSaveString '.csv']);
    
    outputpointspercell(pointsxcell, savePointsFilePath, position, barcodekey)
   

end

function [] = outputpointspercell(pointspercell, savePointsFilePath, position, barcodekey)
    % variables and directory
    numCells = size(pointspercell,1);
    numHybs = size(pointspercell{1},1); % always 1 in this format.
    numChannels = size(pointspercell{1,1}{1,1},2);

    % set up csv file
    fileID = fopen(savePointsFilePath,'w');
    fprintf(fileID,'%s,%s,%s,%s,%s,%s,%s,%s\n', ...
                    'fov', 'cellID', 'hybID', 'geneID', 'x', 'y', 'z', 'intensity');
