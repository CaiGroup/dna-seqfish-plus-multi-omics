function getifv2_Fa2(position, cellnum, Fa2, numChannels, experimentDir, experimentLabel)
...
% Author: Yodai Takei
    ...
    saveGlobalDir = fullfile(experimentDir,'analysis',experimentLabel,'IF-Za2-per-cell');
    ...
    listSavePath = fullfile(saveGlobalDir, ['output-IF-Za2-per-cell-pos' num2str(position) '.csv']);
    fileID = fopen(listSavePath,'w');
    fprintf(fileID,'%s,%s,%s,%s,%s,%s\n', ...
                'fov', 'hyb','channel','target','cellID', 'percentage');
    ...
    for hyb = 1:numChannels % hyb numbers
        for ch = 1:2 % fluorescent channel 1 or 2 for IF
            ...
            for cellID = 1:size(s,1) % for each nucleus
                ...
                fprintf(fileID,'%.0f,%.0f,%.0f,%s,%.0f,%.3f\n',...
                        position, hyb, ch, target, cellID, Fa2{hyb,ch}(cellID)*100);
            end
        end
    end
