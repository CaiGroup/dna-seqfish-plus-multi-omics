function [finalPosList, dotlocations, numpointconsensus, numdotlocations, ...
        ...
        processimagespoints_sequential(experimentDir, experimentName, position, ...
...
% Author: Nico Pierson
    ...
    switch segment
            case 'roi'
                segmentFolder = fullfile('segmentation_pre','cytoROIs');
                segmentPath = fullfile(experimentDir, segmentFolder, ['Pos' num2str(position)], 'RoiSet.zip');
            case 'whole'
                segmentPath = [];
            case 'cellnum'
                segmentPath = fullfile(experimentDir, 'segmentation', ['Pos' num2str(position)], 'cellnum.mat');
    end
    ...
    processallcells_sequential(experimentDir, experimentName, ...
