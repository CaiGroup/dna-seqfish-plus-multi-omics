...
% need the getdirectory, and the get2dtform
    ...
    if zSlices < 16
        ...
        tformTemp = cell(1, zSlices);
        for i = 1:zSlices
            %[optimizer, metric] = imregconfig('monomodal');
            [optimizer, metric] = imregconfig('multimodal');
            optimizer.InitialRadius = initRadius;
            %optimizer.MaximumIterations = 250;%maxIterations;
            tformTemp{i} = imregtform(moving(:,:,i), fixed(:,:,1), 'translation', optimizer, metric);
        end
        
        tformMat = zeros(3, 3, size(zSlices,3));
        for z = 1:zSlices
            tformMat(:,:,z) = tformTemp{z}.T;
        end
        tform = affine2d(median(tformMat, 3));
    else
        if numvarargs <= (argsLimit - 1)
            initRadius = 0.0625;
        end
