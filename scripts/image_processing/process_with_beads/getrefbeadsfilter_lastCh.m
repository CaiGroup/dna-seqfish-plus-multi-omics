function [chaTform, refPointsAligned, intensity, firstThreshold, numRefPoints] ...
    = getrefbeadsfilter_lastCh(experimentDir, position, beadfoldername, roimask, varargin) 
...
%addpath('C:\github\streamline-seqFISH\src\FindThreshold\', '-end');
    ...
    backradius = 3;
    sliding = false;
    searchradius = 6; % changed from 3 on 11/19/20 to recover more fiducials
    processBeads = true;
    typedotsFixed = 'log';
    ...
    [refPoints, fixedThreshold, matches] = matchpointsims(beadImsTemp(numCh:numCh), ...
...
%}
    ...
    %% filter the points
    removeInd = []; % use ch1 removeInd for all
    for ch = 1
        if ~isempty(roimask)
    ...
    listSavePath = fullfile(savePath, ['hyb-points-pos' num2str(position) '.csv']);
